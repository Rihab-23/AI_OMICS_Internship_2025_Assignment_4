...
# -------------------------------------
gse_data <- getGEO("GSE5287", GSEMatrix = TRUE)

# Extract expression data matrix (genes/probes × samples)
expression_data <- exprs(gse_data$GSE5287_series_matrix.txt.gz)


# Extract feature (probe annotation) data
feature_data <-  fData(gse_data$GSE5287_series_matrix.txt.gz)


# Extract phenotype (sample metadata) data
phenotype_data <-  pData(gse_data$GSE5287_series_matrix.txt.gz)
...
getGEOSuppFiles("GSE5287", baseDir = "Raw_Data", makeDirectory = TRUE)

# Untar CEL files if compressed as .tar
untar("Raw_Data1/GSE5287_RAW.tar", exdir = "Raw_Data/CEL_Files")

# Alternatively, unzip if data is compressed as .zip
unzip("Raw_Data1/GSE5287.zip", exdir = "Raw_Data/GSE5287")
...
groups <- factor(phenotype_data$source_name_ch1,
                 levels = c("Responder to Cisplatin chemotherapy", "Non-Responder to Cisplatin chemotherapy"),
                 label = c("Responder", "Non-Pesponder"))
